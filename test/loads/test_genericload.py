'''Module for testing the GenericLoad and Load classes in
cirsoc_402.loads
'''
...
def test_init():
    '''Test __init__ method from GenericLoad class
    '''
    ...
    for f1, f2, f3 in itertools.product(vals, vals, vals):
        for m1, m2, m3 in itertools.product(vals, vals, vals):
            for ref in [ref1, ref2, ref3]:
                load = GenericLoad(xforce=f1, yforce=f2, zforce=f3,
    ...
    # nan behavior
    reference = ReferenceFrame()
    load = GenericLoad(xforce=np.nan)
    ...
    load = GenericLoad(yforce=np.nan, zforce=np.nan)
    ...
    load = GenericLoad(xforce=1, yforce=np.nan, xmoment=np.nan, ymoment=2, zmoment=3)
    ...
    with pytest.raises(TypeError):
        load = GenericLoad(xforce=[2])
    with pytest.raises(TypeError):
        load = GenericLoad(xforce=[2], yforce='1')
    with pytest.raises(TypeError):
        load = GenericLoad(xmoment=[2])
    with pytest.raises(TypeError):
        load = GenericLoad(reference=[2])


def test_repr():
    '''Test __repr__ method from GenericLoad class
    '''
...
def test_eq():
    '''Test __eq__ method from GenericLoad class
    '''
    ...
    for f1, f2, f3 in itertools.product(vals, vals, vals):
        for m1, m2, m3 in itertools.product(vals, vals, vals):
            load1 = GenericLoad(xforce=f1, yforce=f2, zforce=f3,
                                xmoment=m1, ymoment=m2, zmoment=m3)
            load2 = GenericLoad(xforce=f1, yforce=f2, zforce=f3,
                                xmoment=m1, ymoment=m2, zmoment=m3)
            assert load1 == load2
        ...
        for m11, m21, m31 in itertools.product(vals1, vals1, vals1):
                for f12, f22, f32 in itertools.product(vals2, vals2, vals2):
                    for m12, m22, m32 in itertools.product(vals2, vals2, vals2):
                        load1 = GenericLoad(xforce=f11, yforce=f21, zforce=f31,
                                            xmoment=m11, ymoment=m21, zmoment=m31)
                        load2 = GenericLoad(xforce=f12, yforce=f22, zforce=f32,
...
def test_fromarray():
    ...
    for f1, f2, f3 in itertools.product(vals, vals, vals):
        for m1, m2, m3 in itertools.product(vals, vals, vals):
            load = GenericLoad.fromarray(force=[f1, f2, f3],
                                         moment=[m1, m2, m3])
            assert all(load.force == np.array([f1, f2, f3]))
            assert all(load.moment == np.array([m1, m2, m3]))
            ...
            load = GenericLoad.fromarray(moment=[m1, m2, m3])
            assert all(load.force == np.array([0, 0, 0]))
            assert all(load.moment == np.array([m1, m2, m3]))
            assert load.reference == ReferenceFrame()
    ...
    # nan behavior
    reference = ReferenceFrame()
    load = GenericLoad.fromarray(force=[np.nan, np.nan, np.nan])
    assert np.isnan(load.force[0]) == True
    assert np.isnan(load.force[1]) == True
    assert np.isnan(load.force[2]) == True
    assert all(load.moment == np.array([0, 0, 0]))
    ...
    reference = ReferenceFrame()
    load = GenericLoad.fromarray(force=[np.nan, 2, 3.4])
    assert np.isnan(load.force[0]) == True
    ...
    reference = ReferenceFrame()
    load = GenericLoad.fromarray(force=[np.nan, 2, np.nan])
    assert np.isnan(load.force[0]) == True
    assert load.force[1] == 2
    assert np.isnan(load.force[2]) == True
    assert all(load.moment == np.array([0, 0, 0]))
    ...
    reference = ReferenceFrame()
    load = GenericLoad.fromarray(moment=[np.nan, np.nan, np.nan])
    assert np.isnan(load.moment[0]) == True
    ...
    reference = ReferenceFrame()
    load = GenericLoad.fromarray(moment=[np.nan, 2, 3.4])
    assert np.isnan(load.moment[0]) == True
    assert load.moment[1] == 2
    ...
    reference = ReferenceFrame()
    load = GenericLoad.fromarray(moment=[np.nan, 2, np.nan])
    assert np.isnan(load.moment[0]) == True
    assert load.moment[1] == 2
    assert np.isnan(load.moment[2]) == True
    assert all(load.force == np.array([0, 0, 0]))
    ...
    # wrong input type
    with pytest.raises(TypeError):
        load = GenericLoad.fromarray(force='asd')
    with pytest.raises(TypeError):
        load = GenericLoad.fromarray(moment='asd')
    with pytest.raises(TypeError):
        load = GenericLoad.fromarray(force=[2, 'a', 1])
    with pytest.raises(TypeError):
        load = GenericLoad.fromarray(moment=[2, 'a', 1])
    with pytest.raises(ValueError):
        load = GenericLoad.fromarray(moment=[2])
    with pytest.raises(ValueError):
        load = GenericLoad.fromarray(moment=[2, 3, 4, 5])
    with pytest.raises(ValueError):
        load = GenericLoad.fromarray(force=[2])
    with pytest.raises(ValueError):
        load = GenericLoad.fromarray(force=[2, 3, 4, 5])
    with pytest.raises(TypeError):
        load = GenericLoad.fromarray(reference='asd')


def test_shift():
    '''Test shift method from GenericLoad class
    '''

    # no shift
    load1 = GenericLoad(xforce=2, zforce=4, ymoment=-1)
    ...
    ref3.shift(3, 2, 1)
    for ref in [ref1, ref2, ref3]:
        load1 = GenericLoad.fromarray(force=np.random.random(3),
    ...
    for shift in  [-23.5, -0.6, 0, 1.7, 1234]:
        ...
        load = GenericLoad.fromarray(force=force, moment=moment, reference=ref)
    ...
    for shift in  [-23.5, -0.6, 0, 1.7, 1234]:
        shiftedref = copy.deepcopy(ref)
        shiftedref.shift(shift, 0, 0)
        load = GenericLoad.fromarray(force=np.ones(3), moment=np.zeros(3), reference=ref)
    ...
    for ref in [ref1, ref2, ref3]:
        for shift in shifts:
            ...
            load = GenericLoad.fromarray(force=force, moment=moment,
                                        reference=ref)
...
def test_xshift():
    '''Test xshift method from GenericLoad class
    '''
...
def test_yshift():
    '''Test yshift method from GenericLoad class
    '''
...
def test_zshift():
    '''Test zshift method from GenericLoad class
    '''
...
def test_shift_ref():
    '''Test shift_ref method from GenericLoad class
    '''
...
def test_xshift_ref():
    '''Test xshift_ref method from GenericLoad class
    '''
...
def test_yshift_ref():
    '''Test yshift_ref method from GenericLoad class
    '''
...
def test_rotate_along():
    '''Test rotate_along method from GenericLoad class
    '''
    # no shift
    load1 = GenericLoad(xforce=2, zforce=4, ymoment=-1)
    load2 = copy.deepcopy(load1)
    ...
    for theta in np.linspace(-180, 180, 18*4+1):
        ...
        force = [1, 0, 0]
        moment = [1, 0, 0]
        load1 = GenericLoad.fromarray(force=force, moment=moment, reference=ref)
...
def test_xrotate():
    '''Test xrotate method from GenericLoad class
    '''
...
def test_yrotate():
    '''Test yrotate method from GenericLoad class
    '''
...
def test_zrotate():
    '''Test zrotate method from GenericLoad class
    '''
...
def test_rotate_along_ref():
    '''Test rotate_along_ref method from GenericLoad class
    '''
    # no shift
    load1 = GenericLoad(xforce=2, zforce=4, ymoment=-1)
    load2 = copy.deepcopy(load1)
...
def test_xrotate_ref():
    '''Test xrotate_ref method from GenericLoad class
    '''
    # no shift
    load1 = GenericLoad(xforce=2, zforce=4, ymoment=-1)
    load2 = copy.deepcopy(load1)
...
def test_yrotate_ref():
    '''Test yrotate_ref method from GenericLoad class
    '''
    # no shift
    load1 = GenericLoad(xforce=2, zforce=4, ymoment=-1)
    load2 = copy.deepcopy(load1)
...
def test_zrotate_ref():
    '''Test zrotate_ref method from GenericLoad class
    '''
    # no shift
    load1 = GenericLoad(xforce=2, zforce=4, ymoment=-1)
    load2 = copy.deepcopy(load1)
...
def test_to_reference_shift():
    '''Test to_reference method from GenericLoad class for shifts
    '''
...
def test_to_reference_xrotation():
    '''Test to_reference method from GenericLoad class for x rotations
    '''
...
def test_to_reference_yrotation():
    '''Test to_reference method from GenericLoad class for y rotations
    '''
...
def test_to_reference_zrotation():
    '''Test to_reference method from GenericLoad class for z rotations
    '''
...
def test_to_reference_rotation():
    '''Test to_reference method from GenericLoad class for generic
    rotation
    '''
...
def test_to_reference_rotation_shift():
    '''Test to_reference method from GenericLoad class for rotation +
    shifts
    '''
...
def test_to_origin_shift():
    '''Test to_origin method from GenericLoad class for shifts
    '''
...
def test_to_origin_xrotation():
    '''Test to_origin method from GenericLoad class for x rotations
    '''
...
def test_to_origin_yrotation():
    '''Test to_origin method from GenericLoad class for y rotations
    '''
...
def test_to_origin_zrotation():
    '''Test to_origin method from GenericLoad class for z rotations
    '''
...
def test_to_origin_rotation():
    '''Test to_origin method from GenericLoad class for generic
    rotation
    '''
...
def test_resetorigin():
    '''Test resetorigin method from GenericLoad class
    '''
    ref0 = ReferenceFrame()
    coord = [-84.2, -0.4, 0, 3, 18.2]
    thetas = [-180, -134.5, -27, 0, 45, 90, 105.3]
...
def test_add_nan():
    '''Test _add__ method from GenericLoad class for nans inputs
    '''
    ...
    for ref in [ref1, ref2, ref3]:
        ...
        load1 = GenericLoad.fromarray(force=force1, moment=moment1, reference=ref)

        force2 = [np.nan, np.nan, np.nan]
        moment2 = [np.nan, np.nan, np.nan]
        load2 = GenericLoad.fromarray(force=force2, moment=moment2, reference=ref)
        load3 = load1 + load2
...
def test_add_different_datatype():
    '''Test __add__ method from GenericLoad class for different data
    types
    '''
    ...
    for xforce1, yforce1, zforce1, xmoment1, ymoment1, zmoment1 in itertools.product(vals, vals, vals, vals, vals, vals):
        for xforce2, yforce2, zforce2, xmoment2, ymoment2, zmoment2 in itertools.product(vals, vals, vals, vals, vals, vals):
            ...
            load1 = GenericLoad(xforce=xforce1, yforce=yforce1, zforce=zforce1,
                                xmoment=xmoment1, ymoment=ymoment1, zmoment=zmoment1, reference=ref)
            load2 = GenericLoad(xforce=xforce2, yforce=yforce2, zforce=zforce2,
...
def test_add_same_ref():
    '''Test _add__ method from GenericLoad class for same reference
    system
    '''
...
def test_add_different_ref():
    '''Test _add__ method from GenericLoad class for different reference
    systems
    '''
    ...
    for ref in [ref1, ref2, ref3]:
        for x0, y0, z0 in itertools.product(coord, coord, coord):
            for lat, lon, theta in itertools.product(thetas, thetas, thetas): 
                ...
                force2 = np.random.random(3) * 200 - 100
                moment2 = np.random.random(3) * 200 - 100
                load2 = GenericLoad.fromarray(force=force2, moment=moment2, reference=ref_load2)
...
def test_sub_nan():
    '''Test _sub__ method from GenericLoad class for nans inputs
    '''
...
def test_sub_different_datatype():
    '''Test __sub__ method from GenericLoad class for different data
    types
    '''
...
def test_sub_same_ref():
    '''Test _sub__ method from GenericLoad class for same reference
    system
    '''
...
def test_sub_different_ref():
    '''Test _sub__ method from GenericLoad class for different reference
    systems
    '''
...
def test_mul():
    '''Test __mul__ method from GenericLoad class'''
    ...
    for ref in [ref1, ref2, ref3]:
        for xforce, yforce, zforce, xmoment, ymoment, zmoment in itertools.product(vals, vals, vals, vals, vals, vals):
            for multby in vals:
                load1 = GenericLoad(xforce=xforce, yforce=yforce, zforce=zforce,
...
def test_truediv():
    '''Test __truediv__ method from GenericLoad class'''
...
def test_iadd_inputs():
    '''Test __iadd__ method from GenericLoad class for different inputs
    '''
...
def test_iadd_same_ref():
    '''Test __iadd__ method from GenericLoad class for same reference
    system
    '''
...
def test_iadd_different_ref():
    '''Test __iadd__ method from GenericLoad class for different reference
    systems
    '''
...
def test_isub_inputs():
    '''Test __isub__ method from GenericLoad class for different inputs
    '''
...
def test_isub_same_ref():
    '''Test __isub__ method from GenericLoad class for same reference
    system
    '''
...
def test_isub_different_ref():
    '''Test __isub__ method from GenericLoad class for different reference
    systems
    '''
...
def test_imul():
    '''Test __imul__ method from GenericLoad class'''
    ...
    for ref in [ref1, ref2, ref3]:
        for xforce, yforce, zforce, xmoment, ymoment, zmoment in itertools.product(vals, vals, vals, vals, vals, vals):
            for multby in vals:
                load = GenericLoad(xforce=xforce, yforce=yforce, zforce=zforce,
                                    xmoment=xmoment, ymoment=ymoment, zmoment=zmoment,
                                    reference=ref)
                load *= multby
...
def test_itruediv():
    '''Test __itruediv__ method from GenericLoad class'''
